...
<div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
    ...
    <div class="flex flex-wrap gap-2">
        ...
        <a href="{{ route('shop.edit') }}" class="cn-btn-secondary">Edit shop</a>
...
<div class="cn-card overflow-hidden">
    <div class="flex items-center justify-between border-b border-stone-100 px-6 py-4">
        ...
        <a href="{{ route('products.manage') }}" class="text-sm font-medium text-craft-700 hover:underline">View all →</a>
    </div>
    @if ($products->isEmpty())
        <div class="p-8">
            <x-empty-state title="Start your collection" description="Add handmade products for buyers to discover.">
                <x-slot name="icon">✨</x-slot>
                <x-slot name="action"><a href="{{ route('products.create') }}" class="cn-btn-primary">Add first product</a></x-slot>
            </x-empty-state>
        </div>
    @else
                    ...
                    @foreach ($products->take(5) as $product)
                        <tr class="hover:bg-craft-50/50">
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('products.edit', $product) }}" class="text-craft-700 hover:underline">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
