<header class="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-stone-200 bg-white/95 px-4 backdrop-blur dark:border-stone-800 dark:bg-stone-900/95 sm:px-6">
    <div class="min-w-0 flex-1">
        <h1 class="truncate font-display text-lg font-semibold text-stone-900 dark:text-white">@yield('page-title', 'Dashboard')</h1>
        @hasSection('page-subtitle')
            <p class="truncate text-xs text-stone-500">@yield('page-subtitle')</p>
        @endif
    </div>
    <div class="flex shrink-0 items-center gap-2">
        @include('partials.theme-toggle', ['variant' => 'panel'])
        <x-notification-bell />
        @if (auth()->user()->shop)
